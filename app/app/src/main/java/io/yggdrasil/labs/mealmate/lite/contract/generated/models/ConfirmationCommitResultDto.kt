...
sealed interface ConfirmationCommitResultDto {
    @JvmInline
    value class ConfirmationCommitResultDtoOneOfValue(val value: ConfirmationCommitResultDtoOneOf) : ConfirmationCommitResultDto

    @JvmInline
    value class ConfirmationCommitResultDtoOneOf1Value(val value: ConfirmationCommitResultDtoOneOf1) : ConfirmationCommitResultDto
...
object ConfirmationCommitResultDtoSerializer : KSerializer<ConfirmationCommitResultDto> {
    ...
    override fun serialize(encoder: Encoder, value: ConfirmationCommitResultDto) {
        val jsonEncoder = encoder as? JsonEncoder ?: throw SerializationException("ConfirmationCommitResultDto can only be serialized with Json")

        when (value) {
            is ConfirmationCommitResultDto.ConfirmationCommitResultDtoOneOfValue -> jsonEncoder.encodeSerializableValue(ConfirmationCommitResultDtoOneOf.serializer(), value.value)
            is ConfirmationCommitResultDto.ConfirmationCommitResultDtoOneOf1Value -> jsonEncoder.encodeSerializableValue(ConfirmationCommitResultDtoOneOf1.serializer(), value.value)
        }
    }

    override fun deserialize(decoder: Decoder): ConfirmationCommitResultDto {
        val jsonDecoder = decoder as? JsonDecoder ?: throw SerializationException("ConfirmationCommitResultDto can only be deserialized with Json")
