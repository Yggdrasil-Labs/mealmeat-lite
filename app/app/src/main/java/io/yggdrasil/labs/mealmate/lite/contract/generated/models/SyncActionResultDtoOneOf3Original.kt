...
sealed interface SyncActionResultDtoOneOf3Original {
    @JvmInline
    value class AppliedResultDtoValue(val value: AppliedResultDto) : SyncActionResultDtoOneOf3Original

    @JvmInline
    value class RejectedResultDtoValue(val value: RejectedResultDto) : SyncActionResultDtoOneOf3Original
...
object SyncActionResultDtoOneOf3OriginalSerializer : KSerializer<SyncActionResultDtoOneOf3Original> {
    ...
    override fun serialize(encoder: Encoder, value: SyncActionResultDtoOneOf3Original) {
        val jsonEncoder = encoder as? JsonEncoder ?: throw SerializationException("SyncActionResultDtoOneOf3Original can only be serialized with Json")

        when (value) {
            is SyncActionResultDtoOneOf3Original.AppliedResultDtoValue -> jsonEncoder.encodeSerializableValue(AppliedResultDto.serializer(), value.value)
            is SyncActionResultDtoOneOf3Original.RejectedResultDtoValue -> jsonEncoder.encodeSerializableValue(RejectedResultDto.serializer(), value.value)
        }
    }

    override fun deserialize(decoder: Decoder): SyncActionResultDtoOneOf3Original {
        val jsonDecoder = decoder as? JsonDecoder ?: throw SerializationException("SyncActionResultDtoOneOf3Original can only be deserialized with Json")
